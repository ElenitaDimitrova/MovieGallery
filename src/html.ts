import type { DashboardStats, MovieRow, SessionUser } from "./db";

type LayoutOptions = {
  title: string;
  currentPath: "/gallery" | "/dashboard" | "/auth" | "/manage";
  body: string;
  auth?: SessionUser;
  flash?: {
    type: "error" | "success";
...
function navLink(href: string, label: string, currentPath: LayoutOptions["currentPath"]): string {
...
export function renderManagePage(auth: SessionUser | undefined): string {
  if (!auth) {
    return `
